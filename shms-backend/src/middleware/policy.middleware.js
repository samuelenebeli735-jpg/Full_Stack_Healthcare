...
export function authorizePolicy({
  ...
  return async (req, res, next) => {
    ...
    try {
      ...
      const riskLevel = (await isBlocked(req.user.id, ipAddress))
        ? "HIGH"
        : await assessRisk(req.user.id, ipAddress);
      ...
      if (result.reason === "role") {
        ...
        await recordCrossOrgAttempt({ userId: req.user.id });
        await securityAudit({
      ...
      if (sensitivity === "CRITICAL" || sensitivity === "HIGH") {
        await recordSensitiveAccess({ userId: req.user.id });
      }
